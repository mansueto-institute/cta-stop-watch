...
def interpolate_stoptime(trip_df):
    ...
    for i, row in stops_df.iterrows():
            ...
            proportion = row["accumulated_distance"] / row["ping_dist"] if row['ping_dist'] != 0 else 0
    ...
    trip_df['time_diff_seconds'] = trip_df['time_diff'].apply(lambda x: x.total_seconds())
    trip_df['time_diff_seconds'] = trip_df['time_diff_seconds'].replace(0, 1e-9)
    trip_df['speed_mph'] = (trip_df['stop_dist_y'] / 1609) / (trip_df['time_diff_seconds'] / 3600) 

    trip_df = trip_df[['original_index', "seg_combined", 'typ_x', 'bus_stop_time', 'time_diff', 'time_diff_seconds', 'stop_dist_y', 'speed_mph', "unique_trip_vehicle_day_x",
            "stpid_x",
            "p_stp_id_x",
            "geometry_x"]]
    new_names = {'typ_x':'typ', 'stop_dist_y':'stop_dist', 'unique_trip_vehicle_day_x':'unique_trip_vehicle_day', 'stpid_x':'stpid', 'p_stp_id_x':'p_stp_id', 'geometry_x':'geometry'}

    trip_df.rename(columns = new_names, inplace=True)

    #replace values below 1 for speed_mph
    trip_df['speed_mph'] = trip_df['speed_mph'].apply(lambda x: np.nan if x < 1 or x > 100 else x)
    ...
    trip_df['time_diff_seconds'] = trip_df['stop_dist'] / (trip_df['speed_mph'] / 2.23694)

    trip_df.set_index('original_index', inplace=True)
    trip_df = trip_df.merge(ping_times_df, left_index=True, right_index=True, how='outer')
    trip_df.reset_index(drop=True, inplace=True)
    ...
    trip_df.loc[first_ping_index, 'bus_stop_time'] = trip_df.loc[first_ping_index, 'data_time']
    trip_df.loc[last_ping_index, 'bus_stop_time'] = trip_df.loc[last_ping_index, 'data_time']

    #first rows
    for i in range(first_ping_index - 1, -1, -1):
        trip_df.loc[i, 'bus_stop_time'] = trip_df.loc[i + 1, 'bus_stop_time'] - pd.Timedelta(seconds=trip_df.loc[i, 'time_diff_seconds'])
