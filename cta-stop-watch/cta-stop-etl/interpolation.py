...
def interpolate_stoptime(trip_df):
    ...
    trip_df.loc[:, "data_time"] = pd.to_datetime(trip_df["data_time"], format='%Y-%m-%d %H:%M:%S', errors = 'coerce')
    ...
    trip_df["data_time_y"] = pd.to_datetime(trip_df["data_time_y"], format='%Y-%m-%d %H:%M:%S', errors = 'coerce')
    ...
    # stops_df.loc[stops_df['bus_stop_time'].isna(), 'time_diff_seconds'] = (
    #     stops_df.loc[stops_df['bus_stop_time'].isna(), 'stop_dist'] /
    #     (stops_df.loc[stops_df['bus_stop_time'].isna(), 'speed_mph'] / 2.23694)
    # )
    ...
    #new_trip_df["data_time"] = pd.to_datetime(new_trip_df["data_time"], format='%Y-%m-%d %H:%M:%S', errors = 'coerce')
    new_trip_df["bus_stop_time"] = pd.to_datetime(new_trip_df["bus_stop_time"], format='%Y-%m-%d %H:%M:%S', errors = 'coerce')

    # new_trip_df["time"] = new_trip_df["data_time"].combine_first(
    #     new_trip_df["bus_stop_time"]
    # )
